...
#define FILESYS_QT4 		0
#define FILESYS_ANDROID		1
#define FILE_SYSTEM 		FILESYS_QT4
...
#if (FILE_SYSTEM==FILESYS_QT4)
		.description	= "RGB565",//"RGB565"		
#elif (FILE_SYSTEM==FILESYS_ANDROID)
		.description	= "RGB-5-6-5",//"RGB565"
#else
		.description	= "RGB-5-6-5",//"RGB565"
#endif
...
#else
//auto detect only
extern int tq_s_input(struct file *file, void *fh, unsigned int idx);
#define PIX_X			640
#define PIX_Y			480
int fimc_s_input(struct file *file, void *fh, unsigned int i)
{
	...
	fmt.fmt.pix.width       = PIX_X;
	fmt.fmt.pix.height      = PIX_Y;
	fmt.fmt.pix.pixelformat = V4L2_PIX_FMT_RGB32;
	fmt.fmt.pix.sizeimage   = (PIX_X*PIX_Y*32) / 8;
	rtn = tq_s_input(file,fh,i);
	
	fimc_s_fmt_vid_capture(file,fh,&fmt);
	return rtn;
}
#endif
...
int fimc_cropcap_capture(void *fh, struct v4l2_cropcap *a)
{
	...
	if (!ctrl->cam || !ctrl->cam->sd || !ctrl->cap) {
		fimc_err("%s: No capture device.cam(%p),cam->sd(%p),cap(%p)\n", \
		__func__, ctrl->cam, ctrl->cam->sd, ctrl->cap);
		return -ENODEV;
	}
