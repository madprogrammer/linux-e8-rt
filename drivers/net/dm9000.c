...
static void dm9000_schedule_poll(board_info_t *db)
{
	...
	else
	{
		if(!netif_carrier_ok(ndev))
		{
			if(db->phy_mdix_set)
			{
				dm9000_phy_write(ndev, 0, 0x14, 0x0010);
				db->phy_mdix_set = 0;
			}
			else
			{
				dm9000_phy_write(ndev, 0, 0x14, 0x0030);
				db->phy_mdix_set = 1;
			}
...
dm9000_init_dm9000(struct net_device *dev)
{
	...
	if (db->type == TYPE_DM9000A)
	{
		iow(db, DM9000_SMCR, 0x08);
		iow(db, 0x38, 0x2b);
	}
//	else
//	{
...
dm9000_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	board_info_t *db = netdev_priv(dev);
#ifdef TX_IO_CHECK
	int save_mwr, calc_mwr, check_mwr;
#endif
	...
	{
		printk(KERN_INFO "tx point error %04x %04x %04x %04x \n",
...
dm9000_rx(struct net_device *dev)
{
	...
	do {
		...
		{
			printk(KERN_INFO "rx 4 byte point error %04x %04x %04x \n",
		...
		if (GoodPacket &&
			...
			if(calc_mrr != check_mrr)
			{
				printk(KERN_INFO "rx point error %04x %04x %04x %04x \n",
...
dm9000_open(struct net_device *dev)
{
	...
	iow(db, DM9000_GPR, 0x01);	/* REG_1F bit0 activate phyxcer */
	mdelay(1); /* delay needs by DM9000B */

	dm9000_phy_write(dev, 0, MII_BMCR, BMCR_RESET);	/* PHY RESET */
	udelay(100);

	if (db->type == TYPE_DM9000C)
		dm9000_phy_write(dev, 0, 0x1b, 0xe100);
//		dm9000_phy_write(dev, 0, 0x14, 0x0200);
	
	iow(db, DM9000_GPR, 0x00);	/* REG_1F bit0 activate phyxcer */
	do
	{
		udelay(500);
